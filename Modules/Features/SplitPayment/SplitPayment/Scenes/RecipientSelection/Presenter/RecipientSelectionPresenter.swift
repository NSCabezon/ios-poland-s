...
final class RecipientSelectionPresenter {
    ...
    private func preapareViewModel(with recipients: [Recipient]) {
        var recipientCellViewModelsType: [RecipientCellViewModelType] = recipients.map {
            return .recipient(
        ...
        if isRecipientListEmpty {
            recipientCellViewModelsType.append(
                .empty(
                    EmptyCellViewModel(
                        title: localized("generic_label_empty").text,
                        description: localized("#toolbar_split_payment_text_noReceipents").text
                    )
                )
            )
        }
        setViewModelInView(
            RecipientSelectionViewModel(
                cellsViewModel: recipientCellViewModelsType,
                isRecipientListEmpty: isRecipientListEmpty
            )
        )
    }
    
    private func setViewModelInView(_ viewModel: RecipientSelectionViewModel) {
...
extension RecipientSelectionPresenter: RecipientSelectionPresenterProtocol {
    ...
    func viewDidLoad() {
        view?.showLoader()
        let input = GetRecipientsUseCaseInput()
