...
final class RecipientSelectionViewController: UIViewController {
    ...
    private func configureViews() {
        ...
        headerLabel.text = localized("#pl_split_payment_text_recipSanInt")
...
extension RecipientSelectionViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel?.cellsViewModel.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let viewModel = viewModel?.cellsViewModel[safe: indexPath.row] else {
