...
final class PLLoginManagerTests: Tests {
    ...
    func testLoginWithNick() {
        ...
        case .success(let login):
            XCTAssertEqual(String(login.userId), Constants.userId)
    ...
    func testAuthenticateInit() {
        let parameters = AuthenticateInitParameters(userId: Constants.userId, secondFactorData: SecondFactorData(defaultChallenge: DefaultChallenge(authorizationType: .sms, value: Constants.authorizationValue)))
    ...
    func testPubKey() {
        ...
        case .success(let pubkey):
            XCTAssertTrue(pubkey.exponent.isEmpty)
        case .failure(let error):
            print("Error .\(error.localizedDescription)")
            XCTFail("Not getting correct user")
        default:
    ...
    func testAuthenticate() {
        ...
        let parameters = AuthenticateParameters(encryptedPassword: encryptedPassword, userId: "33355343", secondFactorData: SecondFactorDataAuthenticate(response: Response(challenge: Challenge(authorizationType: .sms, value: "21794711"), value: "795031")))
